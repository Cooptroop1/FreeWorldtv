...
import { Film, Loader2, MonitorPlay, Star } from 'lucide-react';
...
export default function DiscoverTab({
  ...
  lastUpdated, setLastUpdated, error, setError, surpriseMe,
  showFilters, setShowFilters, toggleGenreFilter,
  setSelectedGenresFilter, setMinYearFilter, setMaxYearFilter, setMinRatingFilter, setContentType
}: DiscoverTabProps) {
  ...
  // === DISCOVER FETCHING LOGIC (moved here) ===
  useEffect(() => {
    const fetchData = async (isLoadMore = false) => {
      ...
      try {
        ...
        if (newTitles === staticFallbackTitles) setError('Using cached fallback titles');
        ...
        if (!isLoadMore) {
          ...
          setError('Using cached fallback titles');
        }
        setHasMore(false);
      }
      setLoading(false);
  ...
  // TMDB poster fetching (same as before)
  ...
  // Skeleton & HorizontalCarousel (moved here)
    ...
    return (
      <div className="mb-10">
        ...
        <div className="flex gap-4 overflow-x-auto pb-6 px-4 snap-x snap-mandatory scrollbar-hide">
          {isLoading ? Array.from({ length: 8 }).map((_, i) => <SkeletonPoster key={i} />) :
            ...
            }) : <div className="text-gray-500 italic">No titles yet</div>}
        </div>
      </div>
    );
  };
    ...
    <>
      ...
      {!loading && allTitles.length > 0 && (
        <section className="max-w-7xl mx-auto">
          {/* Publisher box, cache badge, hero banner, carousels, All Free Titles grid, JSON-LD — exactly the same as before */}
          <div className="bg-gray-800/50 border border-gray-700 rounded-2xl p-6 mb-8">
            <h2 className="text-2xl font-bold mb-3">Welcome to FreeStream World</h2>
            <p className="text-gray-300 leading-relaxed mb-4">We help you discover completely legal free movies, TV shows and live TV channels...</p>
            <p className="text-gray-300 leading-relaxed">All titles shown are 100% free to watch on the original services...</p>
          </div>

          {lastUpdated && <div className="text-center text-xs text-emerald-400 mb-6">Updated {(() => {
            const diff = Math.floor((Date.now() - new Date(lastUpdated).getTime()) / 3600000);
            return diff === 0 ? 'just now' : `${diff} hour${diff > 1 ? 's' : ''} ago`;
          })()} • Refreshes automatically every 24 hours</div>}
              ...
              {filteredTitles.map((title: any) => {
                ...
                return (
                  <div key={title.id} onClick={() => setSelectedTitle(title)} className="group bg-gray-800/80 rounded-xl overflow-hidden shadow-lg hover:shadow-2xl hover:scale-[1.03] transition-all duration-300 cursor-pointer backdrop-blur-sm relative">
                    {/* poster + heart + share buttons + View Free Sources — exactly the same as your original */}
                    {/* (full block copied from your original file — identical) */}
                    <div className="relative aspect-[2/3] bg-gray-700 overflow-hidden">
                      {title.poster_path ? <Image src={`https://image.tmdb.org/t/p/w500${title.poster_path}`} alt={title.title} fill className="object-cover group-hover:scale-105 transition-transform duration-500" sizes="(max-width: 768px) 50vw, (max-width: 1200px) 25vw, 20vw" quality={85} /> : <div className="w-full h-full flex items-center justify-center"><Film className="w-16 h-16 text-gray-600 group-hover:text-gray-400 transition-colors" /></div>}
